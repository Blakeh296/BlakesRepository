...
/* =CONCATENATE("('",B1,"'),")
  ...
  ALTER TRIGGER CCExpirationCheck
on MemberSubscriptionTransactions
AFTER INSERT 
AS
BEGIN

			DECLARE @SCOPEIDENTITY INT = (SELECT SCOPE_IDENTITY())
			DECLARE @ExpirationDate DATETIME = (SELECT MPI.Expiration FROM MemberPaymentInfo MPI
										   INNER JOIN MemberSubscriptionTransactions MST
										   ON MST.CardID = MPI.CardID
										   WHERE MST.TransactionID = @SCOPEIDENTITY)
	IF @@ROWCOUNT = 1
		BEGIN
			SELECT  MPI.Expiration FROM MemberSubscriptionTransactions MST
			INNER JOIN [dbo].[MemberPaymentInfo] MPI
			ON MPI.MemberID = MST.MemberID
			WHERE MPI.Expiration < GETDATE() AND MST.TransactionID = @SCOPEIDENTITY
